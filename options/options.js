...
(function () {
  ...
  async function sha256Hex(str) {
    ...
    // Note: For stronger protection against offline attacks, consider salting and/or stretching (PBKDF2/Argon2).
  }
  ...
  async function loadSettings() {
    ...
    // Do not prefill the password field for security; leave blank to keep current
  ...
  async function saveSettings() {
    ...
    // Save blocked list to chosen area
    await storage(storageArea).set({ blockedSites: sites });
    // Remember user's choice locally
    await storage("local").set({ blockedStorage: storageArea });

    // Update password hash only if a new value is provided
