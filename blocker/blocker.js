...
(function () {
  ...
  function createOverlay(options = {}) {
    ...
    // IMPORTANT FIX:
    // Do NOT stop propagation in the capture phase — that prevents button clicks from reaching their handlers.
    // Instead, stop in the bubble phase to keep events within the overlay but allow inner handlers to run.
    root.addEventListener("click", (e) => {
      e.stopPropagation(); // contain events within overlay, but after target handlers run
    });

    // Attach overlay to the document root
